...
const PurchasesPage = () => {
  ...
  const handleCompletePurchase = async () => {
    ...
    for (const item of cart) {
      if (
        ...
        alert(
          `Missing details for ${item.medicine_name}. Batch, Expiry, Qty, and Price are required.`,
        );
        return;
      }
    }
  ...
  const handleFileUpload = (e) => {
    ...
    setTimeout(async () => {
      try {
        await purchaseService.ocrextract(file);
      ...
      // Mock Extracted Data
      const mockInvoice = {
        invoice_number: `INV-AUTO-${Math.floor(1000 + Math.random() * 9000)}`,
        supplier_id: suppliers[0]?.id || "",
        items: products.slice(0, 3).map((p) => ({
          product_id: p.id,
          medicine_name: p.medicine_name,
          quantity: Math.floor(Math.random() * 50) + 10,
          purchase_price: p.purchase_price || 120,
      ...
      setSelectedSupplier(mockInvoice.supplier_id);
      setCart(mockInvoice.items);

      setIsScanning(false);
      alert("AI Scan Complete! Form data has been extracted from the invoice.");
    }, 2500);
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3">
              <div className="relative">
                ...
                <Button
                  variant="secondary"
                  className="gap-2 border-primary/20 hover:border-primary/50 transition-all font-bold"
                  onClick={() =>
                  ...
                  {isScanning ? "Scanning Invoice..." : "AI Upload Invoice"}
                </Button>
              </div>
                  ...
                  {cart.length === 0 && (
                    <TableRow>
                      <TableCell
                        ...
                        No medicines added. Search and select items from the
                        sidebar.
                      </TableCell>
                    </TableRow>
                  )}
          ...
          <Card className="flex-1 flex flex-col overflow-hidden">
            ...
            <CardFooter className="border-t bg-muted/10 p-6 flex flex-col gap-4">
              ...
              <Button
                ...
                disabled={
                  cart.length === 0 || !selectedSupplier || !invoiceNumber
                }
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Track inventory restocks and supplier invoices.
          </p>
        </div>
            ...
            {purchases
              ...
              .map((purchase) => (
                <TableRow key={purchase.id} className="hover:bg-muted/20">
                  ...
                  <TableCell>
                    {purchase.supplier_name || `ID: ${purchase.supplier_id}`}
                  </TableCell>
          ...
          <div className="space-y-6 pt-4">
            <div className="grid grid-cols-2 gap-8 text-sm bg-muted/20 p-4 rounded-lg">
              <div className="space-y-1">
                ...
                <p className="text-lg font-bold">
                  {viewingPurchase?.supplier_name || "N/A"}
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-muted-foreground uppercase text-[10px] font-bold tracking-widest">
            ...
            <div className="flex justify-between items-center border-t pt-6">
              <span className="text-muted-foreground">
                Generated Date:{" "}
                {viewingPurchase &&
                  new Date(viewingPurchase.purchase_date).toLocaleString()}
              </span>
              <div className="text-right">
                <p className="text-xs text-muted-foreground font-bold uppercase">
                  Grand Total
                </p>
                <p className="text-3xl font-black text-primary font-mono leading-none mt-1">
